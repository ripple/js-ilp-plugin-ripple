...
function Subscriber (api, sourceSubscriptions, target_uri) {
  this.api = api
...
Subscriber.prototype.postTransfer = function * (transfer) {
  let target = parseURL(this.target_uri)
  let relayRes = yield request({
    method: 'post',
    url: this.target_uri,
    json: true,
    body: {
      // This isn't a real ID, but it needs one anyway
      id: target.protocol + '//' + target.hostname + '/notifications/' + uuid(),
