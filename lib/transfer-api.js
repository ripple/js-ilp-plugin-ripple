...
function TransferAPI (ledger_id, credentials, log) {
...
TransferAPI.prototype.subscribe = function * (onTransaction) {
  ...
  remote.on('transaction', function (notif) {
    co(onTransaction, notif).catch(this.log.error.bind(this.log))
  }.bind(this))
